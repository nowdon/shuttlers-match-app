...
WORKER = r'''export default { async fetch(request, env) {
  ...
  try {
    if (u.pathname === "/session-create") { const v=Number(u.searchParams.get("version")||1), token=u.searchParams.get("token"), placeholder="__session_id_xxx__", serialized=JSON.stringify({bench:[],match_active:false,match_count:0,matches:[],session_id:placeholder}), quoted=JSON.stringify(placeholder); await env.DB.batch([guard('current_match',v),env.DB.prepare("INSERT INTO match_sessions (status,match_count,created_at,creation_token) VALUES ('draft',0,?,?)").bind(ts,token),env.DB.prepare("UPDATE runtime_state SET state_json=REPLACE(?, ?, CAST((SELECT id FROM match_sessions WHERE creation_token = ?) AS TEXT)),version=version+1 WHERE key='current_match' AND version=?").bind(serialized,quoted,token,v)]); return Response.json({ok:true}); }
