...
- ペア作成時はランダム性を残しつつ、過去に組んだことのあるペアをできる限り避けます
...
- 過去に組んだことのあるペアの重複が少なくなるように複数回試行します。
- ペア作成時には `player_score` / `pair_score` による均等化は行わず、ランダム性を残します。
- 完成したペア同士を、`pair_score` が近い対戦になるように並べます。
...
- ペア作成時にはスコアで均等化しすぎないよう、ランダム性を残しています。
