...
- 履歴ダンプとメール送信を best-effort 化し、メール送信に失敗しても全データ削除処理を継続できるように変更
- 全データ削除処理のトランザクション境界を整理し、DB削除と履歴ダンプ・メール送信の責務を分離
- 状態ファイル初期化処理をDB削除トランザクションから分離し、削除後の状態リセットを明確化

### Fixed

- 全データ削除中にDB削除が失敗した場合は rollback し、状態ファイル初期化を行わず警告を表示するように修正
- 履歴ダンプやメール送信の失敗が、参加者データ削除の rollback を引き起こす可能性を修正

## [v1.6.1] - 2026-07-11

### Added

- 社会人用・学生用PayPayリンクに有効期限を設定できる機能を追加
- 管理者トップに、PayPayリンクの期限前日・当日・期限切れ・期限未設定の警告表示を追加
- 試合履歴JSONダンプをSMTPメールに添付して送信する機能を追加
- 管理者設定画面に、履歴ダンプのメール送信有効化と送信先メールアドレスの設定を追加
- SMTPのSTARTTLS、SSL/TLS、暗号化なし接続と、認証あり・なしに対応
- LINE Messaging機能を環境ごとに有効化する `LINE_MESSAGING_ENABLED` 環境変数を追加

### Changed

- SMTPホスト、認証情報、送信元メールアドレスなどを環境変数から取得するように変更
- 手動の履歴ダンプでは、メール送信に失敗しても保存済みJSONを残すように変更
- 「履歴を削除してダンプ」と全データ削除では、メール送信に失敗した場合にDB削除を中止するように変更
- 組み合わせ確定処理を先にコミットし、その後でLINE通知を独立して処理するように変更
- LINE通知の失敗や予期しない例外が、確定済みの組み合わせをロールバックしないように変更
- `LINE_MESSAGING_ENABLED` が未設定の環境では、LINE登録UI、Webhook処理、Push通知、通知管理レコード作成を無効にするように変更
- 古い `config.json` でも、PayPayリンク期限設定と履歴ダンプメール設定を安全な初期値で扱うように変更

### Fixed

- LINE関連の環境変数が未設定の開発環境で、組み合わせ確定処理がLINE通知処理の影響を受ける可能性を修正
- LINE機能が無効な環境でも通知失敗ログや通知管理レコードが作成される問題を修正
- `SMTP_USERNAME` が設定されているのに `SMTP_PASSWORD` が未設定の場合、空のパスワードでSMTP接続を試みる問題を修正

### Notes

- 本番環境でLINE通知を利用する場合は、`LINE_MESSAGING_ENABLED=1` とLINE関連の環境変数を設定してください。
- 開発環境では `LINE_MESSAGING_ENABLED` を設定しなければ、LINE関連の設定なしでアプリを利用できます。
- 履歴ダンプのメール送信を利用する場合は、管理者設定画面で有効化し、SMTP関連の環境変数を設定してください。
- LINEアクセストークン、SMTPパスワードなどのsecretは `config.json` やGitに保存しないでください。
