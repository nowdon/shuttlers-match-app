...
- 「履歴を削除してダンプ」と全データ削除では、JSON保存やメール送信をベストエフォートのバックアップ処理として扱い、JSON保存失敗やメール送信失敗がDB削除を中止しないように変更
- JSON保存またはメール送信に失敗した場合は管理画面とログで警告し、削除処理そのものが失敗した場合だけ削除失敗として扱うように変更
